...
func (qce *QueryCollectionExecutor) Explain(ctx context.Context) (*CollectionRequest, string, error) {
	...
	queryString, err := qce.buildQueryString(expressions)
	if err != nil {
		return nil, "", schema.UnprocessableContentError("failed to evaluate the query", map[string]any{
			"collection": qce.Request.Collection,
			"error":      err.Error(),
		})
	}

	return expressions, queryString, nil
}
...
func (qce *QueryCollectionExecutor) Execute(ctx context.Context) (*schema.RowSet, error) {
	...
	expressions, queryString, err := qce.Explain(ctx)
	if err != nil {
		return nil, err
	}
...
func (qce *QueryCollectionExecutor) buildQueryString(predicate *CollectionRequest) (string, error) {

	if predicate == nil {
		return qce.Request.Collection, nil
	}

	conditions := []string{}
	if len(predicate.LabelExpressions) > 0 {
		keys := make([]string, 0, len(predicate.LabelExpressions))
		for key := range predicate.LabelExpressions {
		...
		for _, key := range keys {
			...
			value, err := qce.getComparisonValue(expr.Value)
			if err != nil {
				return "", err
			}
			condition, ok, err := qce.evalLabelComparisonCondition(expr.Column.Name, expr.Operator, value)
			if err != nil {
				return "", err
			}
			if !ok {
	...
	for _, fn := range predicate.Functions {
		...
		case metadata.Quantile, metadata.LimitRatio, metadata.HistogramQuantile:
			...
			if n != nil {
				if *n < 0 || *n > 1 {
					return "", fmt.Errorf("%s: value should be between 0 and 1, got %v", fn.Key, fn.Value)
				}
		...
		case metadata.Clamp:
			if !utils.IsNil(fn.Value) {
				...
				if err := mapstructure.Decode(fn.Value, &boundaryInput); err != nil {
					return "", fmt.Errorf("%s: invalid clamp input %s", fn.Key, err)
				}
		...
		case metadata.HistogramFraction:
			if !utils.IsNil(fn.Value) {
				...
				if err := mapstructure.Decode(fn.Value, &boundary); err != nil {
					return "", fmt.Errorf("%s: invalid histogram_fraction input %s", fn.Key, err)
				}
		...
		case metadata.LabelJoin:
			if !utils.IsNil(fn.Value) {
				...
				if err := mapstructure.Decode(fn.Value, &input); err != nil {
					return "", fmt.Errorf("%s: invalid label_join input %s", fn.Key, err)
				}
				if input.DestLabel == "" {
					return "", fmt.Errorf("%s: the dest_label must not be empty", fn.Key)
				}
				if len(input.SourceLabels) == 0 {
					return "", fmt.Errorf("%s: the source_labels array must have at least 1 item", fn.Key)
				}
...
func (qce *QueryCollectionExecutor) evalLabelComparisonCondition(name string, operator string, value any) (string, bool, error) {
	switch operator {
	case metadata.Equal:
		return qce.evalStringCondition(name, "=", value)
	case metadata.NotEqual:
		return qce.evalStringCondition(name, "!=", value)
	case metadata.Regex:
		return qce.evalStringCondition(name, "=~", value)
	case metadata.NotRegex:
		return qce.evalStringCondition(name, "!~", value)
	case metadata.In:
		return qce.evalStringSliceCondition(name, "=~", value)
	case metadata.NotIn:
		return qce.evalStringSliceCondition(name, "!~", value)
	default:
		return "", false, fmt.Errorf("%s: unsupported comparison operator `%s`", name, operator)
	}
}
...
func (qce *QueryCollectionExecutor) getComparisonValue(input schema.ComparisonValue) (any, error) {
	...
	case *schema.ComparisonValueVariable:
		if value, ok := qce.Variables[v.Name]; ok {
			return value, nil
		}

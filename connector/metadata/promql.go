...
func buildLabelJoinObjectTypeName(name string) string {
	return name + "LabelJoinInput"
}

func buildLabelReplaceObjectTypeName(name string) string {
	return name + "LabelReplaceInput"
}
		...
		Fields: schema.ObjectTypeFields{
			...
			"regex": schema.ObjectField{
				Description: utils.ToPtr(
					"The regular expression against the value of the source label",
				),
				Type: schema.NewNamedType(string(ScalarString)).Encode(),
			},
		},
...
func createPromQLFunctionObjectFields( //nolint:funlen,maintidx
	name string,
	labelEnumScalarName string,
) schema.ObjectTypeFields {
	typeArrayLabels := schema.NewNullableType(
		schema.NewArrayType(schema.NewNamedType(labelEnumScalarName)),
	).Encode()
	...
	return schema.ObjectTypeFields{
		...
		string(Clamp): schema.ObjectField{
			...
			Type: schema.NewNullableNamedType(string(objectName_ValueBoundaryInput)).
				Encode(),
		},
		...
		string(HistogramStddev): schema.ObjectField{
			Description: utils.ToPtr(
				"Returns the estimated standard deviation of observations in a native histogram, based on the geometric mean of the buckets where the observations lie. Samples that are not native histograms are ignored and do not show up in the returned vector",
			),
			Type: schema.NewNullableNamedType(string(ScalarBoolean)).Encode(),
		},
		...
		string(LabelReplace): schema.ObjectField{
			Description: utils.ToPtr(
				"Matches the regular expression regex against the value of the label src_label. If it matches, the value of the label dst_label in the returned timeseries will be the expansion of replacement, together with the original labels in the input",
			),
			Type: schema.NewNullableNamedType(buildLabelReplaceObjectTypeName(name)).
				Encode(),
		},
